...
class BridgeAgent:
    def __init__(self, cfg: BridgeConfig, log: logging.Logger, room_name: str):
        self._cfg = cfg
        self._log = log
    ...
    async def connect_livekit(self) -> None:
        token_factory = LiveKitTokenFactory(self._cfg)
        connector = LiveKitRoomConnector(self._cfg, self._log, token_factory)
        ...
        def _register_handlers(room: rtc.Room):
            ...
            def _on_track(track, publication, participant):
                if isinstance(track, rtc.RemoteAudioTrack) and not self.remote_audio_track:
                    ...
                    self._log.info("[LiveKit][RTC] Remote audio track subscribed participant=%s sid=%s trackSid=%s",
            ...
            def _on_participant_disc(p):
                ...
                if p.identity and p.identity.startswith("sip-"):
                    self._log.info("[Bridge] SIP leg left -> stopping")
                    self._stop.set()

            @room.on("disconnected")
            def _on_disc():
                self._log.info("[LiveKit][RTC] Room disconnected -> stopping")
                self._stop.set()
